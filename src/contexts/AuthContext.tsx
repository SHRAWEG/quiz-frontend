...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initializeAuth = () => {
      try {
        ...
        const hasPreference =
          getClientCookie(COOKIE_KEYS.HASPREFERENCE) === "true";
        const storedProfilePicture = getClientCookie(
          COOKIE_KEYS.PROFILE_PICTURE
        );
        if (storedToken) {
  ...
  const login = async (data: LoginResDto) => {
    setIsLoading(true);
    try {
      const { accessToken, role, name, email, hasPreference, profilePicture } =
        data;
      setClientCookie(COOKIE_KEYS.TOKEN, accessToken, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setClientCookie(COOKIE_KEYS.EMAIL, email, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setClientCookie(COOKIE_KEYS.NAME, name, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setClientCookie(COOKIE_KEYS.ROLE, role, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setClientCookie(COOKIE_KEYS.HASPREFERENCE, hasPreference.toString(), {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setClientCookie(COOKIE_KEYS.PROFILE_PICTURE, profilePicture, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
      setToken(accessToken);
      setUser({ email, name, role, hasPreference, profilePicture });
  ...
  const logout = () => {
    ...
    deleteClientCookie(COOKIE_KEYS.PROFILE_PICTURE);
  ...
  const updateProfilePicture = (profilePictureUrl: string) => {
    if (user) {
      ...
      setClientCookie(COOKIE_KEYS.PROFILE_PICTURE, profilePictureUrl, {
        maxAge: 60 * 60 * 24,
        path: "/",
      });
    }
